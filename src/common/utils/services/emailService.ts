...
class EmailService {
  ...
  constructor() {
    ...
    if (!host) throw new ServerErrException("No value found for env var: SMTP_HOST");
    if (!port) throw new ServerErrException("No value found for env var: SMTP_PORT");
    if (!user) throw new ServerErrException("No value found for env var: SMTP_USER");
    if (!pass) throw new ServerErrException("No value found for env var: SMTP_PASS");
    if (!from) throw new ServerErrException("No value found for env var: MAIL_FROM");
  ...
  private async sendWithRetry(params: SendEmailParams): Promise<void> {
    ...
    throw new ServerErrException(`Failed to send email after ${this.maxRetries} attempts`);
  }
